...
Start by creating a `week_5b` folder in your assignments repo.
...
<html>
    <head>
     ...
     <body>
        <header>
            <h1>
                Welcome to COVID-19 Vaccination Stories
            </h1>
                <a href="https://docs.google.com/forms/d/e/1FAIpQLSdqVT10bEbUrULMu6Etwj4ZBXGf-LAxcKohAINFbIdZmHS6OA/viewform?embedded=true" id="survey">
                    <button>
                        Take the survey!
                    </button>
                </a>
...
function addMarker(data){
    if(data.doyouspeakenglishfluently == "Yes"){
        L.marker([data.lat,data.lng]).addTo(myMap).bindPopup(`<h2>Speaks English fluently</h2>`)
        createButtons(data.lat,data.lng,data.location)
    }
    else{
        L.marker([data.lat,data.lng]).addTo(myMap).bindPopup(`<h2>Speak other languages</h2>`)
        createButtons(data.lat,data.lng,data.location)   
        // Bonus:    
...
function addMarker(data){
    if(data.doyouspeakenglishfluently == "Yes"){
        speakFluentEnglish.addLayer(L.marker([data.lat,data.lng]).addTo(myMap).bindPopup(`<h2>Speak English fluently</h2>`))
        createButtons(data.lat,data.lng,data.location)
        }
    else{
        speakOtherLanguage.addLayer(L.marker([data.lat,data.lng]).addTo(myMap).bindPopup(`<h2>Speak other languages</h2>`))
        createButtons(data.lat,data.lng,data.location)
    }
    return data.timestamp
}
...
Awesome! Now let's see our beautiful layers!
 ...
 We also need to delete the line `.addTo(myMap)` after each marker, because we will add the `featureGroup` to the map instead of individual markers:
...
<details>
<summary><b>Answer</b></summary>
Since we removed `addTo(myMap)`, we now need to add the `group layers` to to map!!
</details>
...
function formatData(theData){
        ...
        console.log(formattedData)
        formattedData.forEach(addMarker)
        speakFluentEnglish.addTo(myMap) // add our layers after markers have been made
...
## Bonus Assignment #2 - Conditional Visual Importance
### Due 7/26
