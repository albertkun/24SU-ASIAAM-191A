...
Recall from this week's pre-lab [reading](https://developer.mozilla.org/en-US/docs/Learn/Getting_started_with_the_web/The_web_and_web_standards#html_css_and_javascript) that a webpage is like a house:
...
We will start this lab off with a simpler Leaflet template code:
...
Is there anything different in `index.html` and `init.js` above than from last week?
<details>
...
### `index.html`
- Does not reference to an external `style.css` 
- Uses one in-line css style for the map.
- The `ID` for the map div has been changed from `map` to `mapArea`.

### `init.js`
- The map variable is renamed to `myMap` to make it more distinct from Leaflet's `map`
</details>

Let's talk a little about the reason for these  changes!
...
-  `var` is `VARy` problematic because this box can be both locked and unlocked 
...
let home = L.marker([37.7409, -122.484]).addTo(myMap)
...
Replace the hard coded values of `const myMap = L.map('mapArea').setView([34.0709, -118.444], 5);` with variables.
...
// const myMap = L.map('mapArea').setView(myMap.center, myMap.zoom);
```
</details>
...
```
**Important:** The order of the parameters (`lat`,`lng`,`message`) is the **SAME** order that the function reads them!! Try swapping the locations of `37` and  `-122` to see what I mean.
